...
Using the decomposition from 4e, we bound the attention output difference $\Delta_t^{(l)}$ as follows:
\begin{equation}
...
\begin{itemize}
    \item \textbf{Term 1 (Application of 4h and 4i):} Sub-question \textbf{4h} establishes that individual attention weights change by $\mathcal{O}(1/T^2)$. Since representations $\|x_{t'}^{(l)}\|$ are bounded by a constant (from \textbf{4b}), sub-question \textbf{4i} concludes the sum of these $T$ differences is $\mathcal{O}(1/T)$.
    \item \textbf{Term 2 (Application of 4d and Hypothesis):} We split the sum at $t'=t^*$. At the flipped bit, $s_{t,t^*}^{\prime(l)} \|x_{t^*}^{(l)} - x_{t^*}^{\prime(l)}\| = \mathcal{O}(1/T) \cdot \mathcal{O}(1) = \mathcal{O}(1/T)$ using the weight bound from \textbf{4d} and the hypothesis. For $t' \neq t^*$, the hypothesis states $\|x_{t'} - x'_{t'}\| = \mathcal{O}(1/T)$, so the weighted sum is also $\mathcal{O}(1/T)$.
\end{itemize}
Summing these yields $\Delta_t^{(l)} = \mathcal{O}(1/T) + \mathcal{O}(1/T) = \mathcal{O}(1/T)$, proving Equation (55).

\textbf{Step 2: Proving (56) for Layer $l+1$ (Stability of the Representation)}
The next representation is $x_t^{(l+1)} = f(z_t^{(l)}, x_t^{(l)})$. Applying the Lipschitz continuity of $f$:
\begin{equation}
